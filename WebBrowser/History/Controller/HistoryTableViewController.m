...
- (void)scrollViewContentSizeDidChange:(NSDictionary *)change{
    ...
    self.bottomNoMoreLabel.frame = CGRectMake(0, yContentHeight, self.tableView.frame.size.width, 30);
}
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change{
    ...
    if (yOffset > 0 && yOffset + self.tableView.frame.size.height + kHistoryTableViewThreshold > self.tableView.contentSize.height) {
        if (!self.noMoreData) {
            self.isLoading = YES;
            [self.historyDataManager getMoreDataWithCompletion:^(NSArray<NSIndexPath *> *indexPaths, BOOL isNoMoreData){
                if (self) {
                    self.noMoreData = isNoMoreData;
                    [self addNoMoreDataViewIfNeeded];
                    [self reloadBeforeHistorySectionIfNeeded:indexPaths];
                    self.isLoading = NO;
                }
            }];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [[DelegateManager init] performSelector:@selector(browserContainerViewLoadWebViewWithSug:) arguments:@[itemModel.url] key:kDelegateManagerBrowserContainerLoadURL];
    [self.navigationController popViewControllerAnimated:NO];
}
