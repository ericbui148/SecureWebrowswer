...
- (instancetype)initWithCompletion:(HistoryDataCompletion)completion{
    if (self = [super init]) {
        ...
        _historySQLiteManager = [HistorySQLiteManager init];
        [self initData];
    }
    return self;
}
...
- (void)initData{
    ...
    HistoryDataTempCompletion completion = ^{
        [self.historySQLiteManager getHistoryDataByLimit:kLimit offset:self.offset handler:^(NSMutableArray<HistoryItemModel *> *array){
            // always called in main thread
            STRONG_REF(self);
            if (self) {
                [self updateNoMoreDataWithCount:array.count];
                [self addSectionDataWithArray:array date:kBeforeDate];
                if (self.completion) {
                    self.completion(self.isNoMoreData);
                }
            }
        }];
    };
    ...
    [self.historySQLiteManager getTodayAndYesterdayHistoryDataWithHandler:^(NSMutableArray<HistoryItemModel *> *today, NSMutableArray<HistoryItemModel *> *yesterday){
        ...
        STRONG_REF(self);
        if (self) {
            [self addSectionDataWithArray:today date:kTodayDate];
            [self addSectionDataWithArray:yesterday date:kYesterdayDate];
            completion();
        }
    }];
}
...
- (void)getMoreDataWithCompletion:(HistoryDataLoadMoreCompletion)completion{
    ...
    [self.historySQLiteManager getHistoryDataByLimit:kLimit offset:self.offset handler:^(NSArray<HistoryItemModel *> *array){
        ...
        if (self) {
            ...
            if (array.count > 0) {
                NSInteger section = self.historyArray.count - 1;
                NSInteger startIdx = self.historyArray[section].itemsArray.count;
            ...
            [self addBeforeHistoryDataWithArray:array];
            [self updateNoMoreDataWithCount:array.count];
            
            if (completion) {
                completion(indexPaths, self.isNoMoreData);
            }
        }
    }];
}
