...
- (void)getImageWithURL:(NSURL *)url completion:(void (^)(UIImage *, NSError *))completion{
    [[SDWebImageManager sharedManager] loadImageWithURL:url options:SDWebImageFromCacheOnly progress:nil completed:^(UIImage *image, NSData *data, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL){
...
- (NSURLSessionDataTask *)GET:(NSString *)URLPath parameters:(NSDictionary *)parameters modelClass:(Class)modelClass success:(void (^)(NSURLSessionDataTask *, BaseResponseModel *))success failure:(void (^)(NSURLSessionDataTask *, BaseResponseModel *))failure{
    ...
    NSURLSessionDataTask *op = [_afManager GET:URLPath parameters:parameters progress:nil success:^(NSURLSessionDataTask *task,id responseObject){
            if ([modelClass isSubclassOfClass:[BaiduSugResponseModel class]]) {
                [self parseJSONNoKeySugSuccessResponse:responseObject task:task modelClass:modelClass success:success failure:failure];
            }
            else
                [self parseSuccessResponse:responseObject task:task modelClass:modelClass success:success failure:failure];
    }failure:^(NSURLSessionDataTask *task,NSError *error){
            BaseResponseModel *baseModel = [self failureResponseModelWithTask:task];
            dispatch_main_safe_async(^{
                if (failure) {
