...
- (void)viewDidLoad {
    ...
    [[KeyboardHelper init] addDelegate:self];
...
- (void)commonUIInit{
    ...
    self.searchInputView = ({
        ...
        inputView.frame = CGRectMake(self.view.bounds.origin.x, self.view.frame.size.height - SEARCH_INPUTVIEW_HEIGHT, self.view.frame.size.width, SEARCH_INPUTVIEW_HEIGHT);
...
- (void)textFieldTextDidChange:(NSNotification *)notify{
    ...
    if (textField.text.length > 0) {
        ...
        WEAK_REF(self)
        self.bdRecoTask = [[HTTPClient init] getSugWithKeyword:textField.text success:^(NSURLSessionDataTask *task, BaseResponseModel *model){
            STRONG_REF(self_)
            if (self__) {
...
- (void)changeSearchInputViewPoint:(KeyboardState *)state{
    ...
    CGFloat keyBoardEndY = self.view.frame.size.height - [state intersectionHeightForView:self.view];
    ...
    [UIView animateWithDuration:state.animationDuration animations:^{
        ...
        self.searchInputView.center = CGPointMake(self.searchInputView.center.x, keyBoardEndY - self.searchInputView.frame.size.height/2.0f);
    }];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (text) {
        [[DelegateManager init] performSelector:@selector(browserContainerViewLoadWebViewWithSug:) arguments:@[text] key:kDelegateManagerBrowserContainerLoadURL];
    }
