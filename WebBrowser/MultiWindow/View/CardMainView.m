...
- (void)commonInitWithFrame:(CGRect)frame{
    ...
    self.cardBottomView = ({
        CardMainBottomView *bottomView = [[CardMainBottomView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - BOTTOM_TOOL_BAR_HEIGHT, self.frame.size.width, BOTTOM_TOOL_BAR_HEIGHT)];
...
- (void)reloadCardMainViewWithCompletionBlock:(CompletionBlock)completion{
    [[TabManager sharedInstance] setMultiWebViewOperationBlockWith:^(NSArray<WebModel *> *modelArray){
            ...
            [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:self.cardArr.count - 1 inSection:0] atScrollPosition:UICollectionViewScrollPositionBottom animated:NO];
            if (completion) {
                completion();
            }
    }];
...
- (CardCollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.closeBlock = ^(NSIndexPath *index){
            [self.cardArr removeObjectAtIndex:index.item];
