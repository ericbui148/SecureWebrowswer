...
- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath{
    if (self.tableState == BookmarkTableStateSectionEdit) {
        ...
        [self.dataManager moveRowAtIndexPath:sourceIndexPath toIndexPath:destinationIndexPath completion:^(BOOL success){
            if (success && sourceIndexPath.section != destinationIndexPath.section && destinationIndexPath.section <  self.sectionInfoArray.count && !self.sectionInfoArray[destinationIndexPath.section].open) {
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    ...
                    [self sectionHeaderView:headerView sectionOpened:destinationIndexPath.section isMove:YES];
                });
            }
        }];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!tableView.editing) {
        ...
        if (model.url.length > 0) {
            [[DelegateManager init] performSelector:@selector(browserContainerViewLoadWebViewWithSug:) arguments:@[model.url] key:kDelegateManagerBrowserContainerLoadURL];
